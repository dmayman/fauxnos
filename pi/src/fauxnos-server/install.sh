...
SERVICE
  ...
  - id: analog
    ...
    # Resolve the persisted display_name from server_config.json so a
    # re-run of install.sh after the user renamed the server in the UI
    # doesn't silently clobber the chosen name back to "Server". Falls
    # back to "Server" only on a truly fresh install where fauxnos000
    # hasn't been registered yet.
    local fauxnos000_display_name
    fauxnos000_display_name=$(python3 -c "
import json, sys
try:
    c = json.load(open('$INSTALL_DIR/server_config.json'))
    for x in c.get('clients', []):
        if x.get('id') == 'fauxnos000':
            print(x.get('name') or 'Server')
            break
    else:
        print('Server')
except Exception:
    print('Server')
" 2>/dev/null)
    fauxnos000_display_name=${fauxnos000_display_name:-Server}
...
device:
  name: fauxnos000
  mac: "$client_mac"
  display_name: $fauxnos000_display_name
